# tests\backend\gui\_api\video\test_api_video_resize.py
...
from file_conversor.backend.gui._api.video.resize import api_video_resize, EXTERNAL_DEPENDENCIES
...
class TestAPIVideoResize:
    def test_resize(self, tmp_path: Path):
        url = FlaskRoute.get_url(api_video_resize)
        ...
        assert (tmp_path / "test_resized.mp4").exists()
