...
class TestScoopPackageManager:
    def test_get_pkg_manager_installed(self) -> None:
        ...
        if pkg_manager._get_pkg_manager_installed():  # pyright: ignore[reportPrivateUsage]  # noqa: SLF001
            assert Path(pkg_manager._get_pkg_manager_installed() or '').with_suffix("").name == "scoop"  # pyright: ignore[reportPrivateUsage]  # noqa: SLF001
