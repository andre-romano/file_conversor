...
from file_conversor.backend.gui.image._dom_page import image_index_nav_item, image_info_nav_item
...
def PageImageInfo():
    return PageForm(
        ...
        api_endpoint=f"{url_for('api_image_info')}",
        nav_items=[
            home_nav_item(),
            image_index_nav_item(),
            image_info_nav_item(active=True),
        ],
        _title=f"{_('Image Info')} - File Conversor",
    )
