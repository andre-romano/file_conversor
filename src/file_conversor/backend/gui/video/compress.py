...
from file_conversor.backend.gui.video._dom_page import video_compress_nav_item, video_index_nav_item
...
def PageVideoCompress():
    return PageForm(
        ...
        api_endpoint=f"{url_for('api_video_compress')}",
        nav_items=[
            home_nav_item(),
            video_index_nav_item(),
            video_compress_nav_item(active=True),
        ],
        _title=f"{_('Video Compress')} - File Conversor",
    )
