# src\file_conversor\cli\progress_manager_rich.py


# user-provided
from file_conversor.cli._utils.rich import get_progress_bar
...
class ProgressManagerRich:
    ...
    def completed_files(self, value: int):
        if value > self._total_out_files:
            raise RuntimeError(f"ProgressManager - Completed '{value}' files > '{self._total_out_files}' total out files")
